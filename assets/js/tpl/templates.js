...
var out='<h3> Settings </h3>';if(it.length > 0){out+='<ul class="settings-group-list">';var arr1=it;if(arr1){var group,index=-1,l1=arr1.length-1;while(index<l1){group=arr1[index+=1];out+='<li><a href="/settings/'+( group.name)+'"> '+( group.name )+'</a></li>';} } out+='</ul>';}else{out+='<p>There\'s nothing to configure right now.<br/>Later on, you\'ll be able to set the backend url for persisting your data in the cloud.<br/>Some UI options might also be available, like the editor preview position (bottom or right).</p>';}return out;
};
  tmpl['settings-show']=function anonymous(it) {
var out='<h3>'+( it.name )+'</h3><a href="/clear/config/'+( it.name )+'"> Return to default </a>'; for (var prop in it.properties) { out+='<div class="param">'+( prop )+'undefined</div>'; } return out;
};
